import { DEFAULT_DATE_FORMAT, FREE_NAME_PRODUCT_ID, PRODUCT_STATUS, SPECIAL_PRODUCT_ID_LIST } from '../config/constants';
import { getUnixTime } from 'date-fns';
import { utcToZonedTime, format } from 'date-fns-tz';
import { ACL_PERMISSION } from '../config/constants';
...
export const getTodayLastMonth = () => {
  let date = new Date();
  ...
  return getUnixTime(date) - 86400 * dayMonth;
}
...
export const generateProjection = projectFieldList => {
  let projection = {};
  projectFieldList.forEach(x => {
    projection[x] = 1;
  })
  return projection;
}
export const checkPermission = (permission, collection, user) => {
  return (ACL_PERMISSION[permission] & parseInt(user.permission[collection], 2)) == ACL_PERMISSION[permission];
}
...
export const randomNumber = (max, min = 0, type = 'int') => {
...
export const isOverweight = (id) => !id;

export const isSpecialProduct = (id) => SPECIAL_PRODUCT_ID_LIST.includes(id);

export const isAsscociatedProduct = (product) => product?.status === PRODUCT_STATUS.associated;