import { OFFER_TYPE, SPECIAL_PRODUCT_ID_LIST } from 'app/constants';
import { checkMembership } from './customer.helper';
import { formatCurrency, isFreeNameProduct } from './general.helper';

export const getPriceInStore = (customer: any, product: any) => {
  if (product.store_price_list) {
    let storePrice = product.store_price_list[customer?.store_id ?? null] ?? null;
    if (storePrice) {
      return storePrice;
    }
  }
...
export const processOrderItemList = (items: any, customer: any, orderInfo: any = null, throwError: boolean = false) => {
  ...
  items.forEach((item: any) => {
    ...
    if (SPECIAL_PRODUCT_ID_LIST.includes(cart._id)) {
      cart.position = -1;
    }
