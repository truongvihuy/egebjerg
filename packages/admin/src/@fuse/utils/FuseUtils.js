...
import { AUTH_PERMISSION, USER_GROUP_ADMIN } from 'app/constants';
...
class FuseUtils {
	...
	static hasPermission(pathname = '/', user) {
		if (!user) {
			if (pathname === '/login') {
				return true;
			}
		...
		if (USER_GROUP_ADMIN === user.user_group_id) {
			return true;
		}

		const permissionKey = pathname.slice(1).replace('-', '_');
		if (user.permission[permissionKey]) {
			return (user.permission[permissionKey] & AUTH_PERMISSION.access) === AUTH_PERMISSION.access;
		}
		...
		return !data
			...
			: data.reduce((list, entry) => {
				...
				if (predicate(entry)) {
					...
					clone = { ...entry };
					// If not delete, children still show althout that item has no permission
